...
#define XRM117XX_ISR_REG		(0x02) /* Interrupt Status */
...
#define XRM117XX_MSR_DCD_BIT		(1 << 3) /* Delta CD Carrier Detect
			  ...
			  * - only on 75x/76x
			  */
#define XRM117XX_MSR_CTS_BIT		(1 << 4) /* CTS */
#define XRM117XX_MSR_DSR_BIT		(1 << 5) /* DSR (IO4)
			  * - only on 75x/76x
			  */
#define XRM117XX_MSR_RI_BIT		(1 << 6) /* RI (IO7)
			  * - only on 75x/76x
			  */
#define XRM117XX_MSR_CD_BIT		(1 << 7) /* CD (IO6)
			  * - only on 75x/76x
			  */
