...
#ifndef PORT_XR20M1172
#define PORT_XR20M1172   128
#endif
...
#define to_xrm117x_port(p,e) ((container_of((p), struct xrm117x_port, e)))
...
#if 0
static const char* xrm117x_reg_r[] = {
	"RHR-ro", "IER-rw", "ISR-ro", "LCR-rw", "MCR-rw", "LSR-ro", "MSR-ro/TCR-rw", "SPR-rw/TLR-rw", "TXLVL-rw", 
	"RXLVL-rw", "IODir-rw", "IOState-rw", "IOIntEna-rw", "Reserved-err", "IOControl-rw", "EFCR-rw" 
}; 

static const char* xrm117x_reg_w[] = {
	"THR-wo", "IER-rw", "FCR-wo", "LCR-rw", "MCR-rw", "?05-err", "?06-err/TCR-rw", "SPR-rw/TLR-rw", "TXLVL-rw",
	"RXLVL-rw", "IODir-rw", "IOState-rw", "IOIntEna-rw", "Reserved-err", "IOControl-rw", "EFCR-rw" 
}; 

static const char* xrm117x_reg_name(u8 reg, u8 read)
{
		if (reg < 16)
			return (read ? xrm117x_reg_r[reg] : xrm117x_reg_w[reg]);
		return "Unknown";
} 
#endif
static u8 xrm117x_port_read(struct uart_port *port, u8 reg)
{
	...
	ssize_t		status;
	u8			result;
	//printk("  xrm1172 spi port read1  (%s) reg<0x%02x> \n", xrm117x_reg_name(reg, 1), reg);
	Xmutex_lock(&s->mutex_bus_access);
	cmd = (0x80 | (reg<<3) | (port->line << 1));
	status = spi_write_then_read(spi_dev, &cmd, 1, &result, 1);

	Xmutex_unlock(&s->mutex_bus_access);
	if(status < 0) {
		printk("Failed to xrm117x_port_read error code \n");
	}
#if 0 
	else
		printk("  xrm1172 spi port read2  (%s) reg<0x%02x> data<0x%02x>\n", xrm117x_reg_name(reg, 1), reg, result);
#endif
	return result;
...
static void xrm117x_port_write(struct uart_port *port, u8 reg, u8 val)
{
	...
	if(status < 0) {
		...
		printk("  xrm1172 spi port write (%s) reg<0x%02x> data<0x%02x>\n", xrm117x_reg_name(reg, 0), reg , spi_buf[1] );
#endif
}
...
void xrm117x_raw_write(struct uart_port *port,const void *reg,unsigned char *buf,int len)
{
	...
	Xmutex_unlock(&s->mutex_bus_access);
	if(status < 0) {
		printk("Failed to xrm117x_raw_write Err_code %ld \n", status);
	}
...
static void xrm117x_handle_tx(struct uart_port *port)
{
	...
	if (likely(to_send)) {
		...
		if (txlen == 0) {
			txlen = xrm117x_port_read(port, XRM117XX_RXLVL_REG);
		    return;
		}
...
static void xrm117x_port_irq(struct xrm117x_port *s, int portno)
{
	...
	do {
		...
		isr = xrm117x_port_read(port, XRM117XX_ISR_REG);

		if (isr & XRM117XX_IIR_NO_INT_BIT)
			break;

		isr &= XRM117XX_IIR_ID_MASK;
		...
		case XRM117XX_IIR_XOFFI_SRC:
			...
			if (rxlen)
				xrm117x_handle_rx(port, rxlen, isr);
			break;

		case XRM117XX_IIR_CTSRTS_SRC:
			msr = xrm117x_port_read(port, XRM117XX_MSR_REG);
			if (msr & XRM117XX_MSR_DCTS_BIT) { //status has changed
				uart_handle_cts_change(port, (msr & XRM117XX_MSR_CTS_BIT)); //pass the current state
		...
		default:
			dev_err(port->dev,
					"Port %i: Unexpected interrupt: 0x%x",
					port->line, isr);
			break;
		}
	} while (1);
...
#else
static void xrm117x_ist(struct kthread_work *ws)
{
	struct xrm117x_port *s = to_xrm117x_port(ws, irq_work);
	int i;
...
static irqreturn_t xrm117x_irq(int irq, void *dev_id)
{
	struct xrm117x_port *s = (struct xrm117x_port *)dev_id;
						 ...
						 int irq, unsigned long flags)
{
	...
	unsigned long *pfreq = 0; //dev_get_platdata(dev);
	...
	if (IS_ERR(s->clk)) {
	    dev_err(dev, "Error at devm_clk_get\n");
		if (pfreq)
			freq = *pfreq;
		else
	    ...
	    dev_dbg(dev, "clk found: rate =%li\n", freq);
	}
#else
	//freq = 24000000; //14745600;//Fixed the clk freq
	freq = 14745600; // 9600 baud
#endif
	s->devtype = devtype;
	dev_set_drvdata(dev, s);
	
	kthread_init_worker(&s->kworker);
	kthread_init_work(&s->irq_work, xrm117x_ist);
	s->kworker_task = kthread_run(kthread_worker_fn, &s->kworker,"xrm117x");
	...
	for (i = 0; i < devtype->nr_uart; ++i) {
		...
		xrm117x_dump_register(&s->p[i].port);
...
#endif
	ret = devm_request_irq(dev, irq, xrm117x_irq, IRQF_ONESHOT  | flags, dev_name(dev), s);
...
#ifdef CONFIG_OF
static int xrm117x_spi_probe_dt(struct spi_device *spi, struct xrm117x_devtype *devtype)
{
	...
	/* Number of supported UART devices - 2 devices by default */
	ret = of_property_read_u32(np, "nr-uart", &value);
	if (!ret && value > 0 && value < 3) {
		devtype->nr_uart = value;
	}
	dev_dbg(&spi->dev, "nr-uart: %d\n", devtype->nr_uart);
...
static int xrm117x_spi_probe(struct spi_device *spi)
{
	...
	ret = xrm117x_spi_probe_dt(spi, devtype);
	if (ret)
		return ret;
#if 0
	printk("xrm117x: spi max freq =%i\n", spi->max_speed_hz);
	printk("xrm117x: spi mode=0x%06x\n", spi->mode);
	printk("xrm117x: spi bits per word=%i\n", spi->bits_per_word);
#endif	
			   ...
			   const struct i2c_device_id *id)
{
	...
	/*
	if (i2c->dev.of_node) {
		const struct of_device_id *of_id = of_match_device(xrm117x_match_table, &i2c->dev);
