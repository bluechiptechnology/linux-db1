    ...
    struct sn65dsi83 *sn65dsi83)
{
    ...
    if (backlight) {
        ...
        dev_err(dev, "backlight not defined\n");
        backlight = 0;
    }
    ...
    sn65dsi83->brg->gpio_panel = devm_gpiod_get(dev, "panel", GPIOD_OUT_LOW);
    if (IS_ERR(sn65dsi83->brg->gpio_panel)) {
        dev_err(dev, "failed to parse panel gpio");
        return PTR_ERR(sn65dsi83->brg->gpio_panel);
    }
...
static int sn65dsi83_attach_dsi(struct sn65dsi83 *sn65dsi83)
{
    ...
    dev_dbg(dev, "attach dsi: lanes=%i\n", dsi->lanes);
    ...
    if (ret < 0) {
        dev_err(dev, "failed to attach dsi to host (%i)\n", ret);
