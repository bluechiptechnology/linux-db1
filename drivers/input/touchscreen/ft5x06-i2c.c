...
#ifdef MODULE
module_param(enable_pcap_multitouch, int, 0);
#else
...
static int ft5x06_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (NULL == ts)
    {
        printk(KERN_ERR "%s() - ERROR: Could not allocate %ld bytes of kernel memory for ft5x06 struct.\n", __FUNCTION__, sizeof(struct ft5x06));
        retval = -ENOMEM;
        goto error_return;
    }
